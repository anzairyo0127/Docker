...
後述のDocker Composeを使用することで

ホストOSのフォルダ、ディレクトリがコンテナ内のディレクトリにマウントされます。

つまり、ホストOSのフォルダの中身が、コンテナ内に共有されます。

ここでの設定は共有先のコンテナ内のフォルダ（ディレクトリ）の名前になります。

ちなみに、Docker Toolboxでは`/`直下では動作しないようです。

その場合は`/XXXXXXX/YYYYYYY`のように一つディレクトリを挟むようにしてください

例`ARG enviwork="/iroha/nihoheto"`
...
- `Dockerfile`の`マウントポイントの指定`と同じ値にしてください。-
